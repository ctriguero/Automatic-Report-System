...
int main( int argc, const char* argv[] )
{
    ...
    std::cout << BOLDYELLOW << "          /__/  \\         Carles Triguero 2024      " << RESET << std::endl ;
...
//    MdeUp //satisfactory
    ...
    topikini.push_back ("topic of") ;
    topikini.push_back ("topics of") ;
    int toto, alltopics;
    ...
    std::string T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,marks,grade,A,B,C,D,E,predicted ;
    std::string tes1,tes2,tes3,tes4,tes5,tes6 ;
    std::string home1,home2,home3,home4,home5,home6,home7 ;
    
    
    // Numerical variables
    int sus;
    int Q1,Q2,Q3,Q4,Q5,Q6,Q7,Q8,Q9,Q10,mark ;
    int homework1,homework2,homework3,homework4,homework5,homework6,homework7 ;
    ...
    // reading first line to get the topic lavels as strings NOT NUMERICAL
    getline(InputFile, line);
    std::stringstream aa(line);
    aa >> group >> surename >> name >> sex >> T1 >> T2 >> T3 >> T4 >> T5 >> T6 >> T7 >> T8 >> T9 >> T10 >> marks >> grade >> predicted >> tes1 >> tes2 >> tes3 >> tes4 >> tes5 >> home1 >> home2 >> home3 >> home4 >> home5 >> home6 >> home7 ;


    cout << "File headers" << endl ;
    cout << YELLOW << group << " " << surename << " " << name << " " << sex << " " << T1 << " " << T2 << " " << T3 << " " << T4 << " " << T5 << " " << T6 << " " << T7 << " " << T8 << " " << T9 << " " << T10 << " " << marks << " " << grade << " " << predicted << " " << tes1 << " " << tes2 << " " << tes3 << " " << tes4 << " " << tes5 << " " << home1 << " " << home2 << " " << home3 << " " << home4 << " " << home5 << " " << home6 << " " << home7 << " " << RESET << endl ;
    cout << endl ;
    cout << endl ;
    cout << endl ;
    cout << endl ;
...
{
    std::stringstream aa(line) ;
    aa >> group >> surename >> name >> sex >> Q1 >> Q2 >> Q3 >> Q4 >> Q5 >> Q6 >> Q7 >> Q8 >> Q9 >> Q10 >> mark >> grade >> predicted >> test1 >> test2 >> test3 >> test4 >> test5 >> homework1 >> homework2 >> homework3 >> homework4 >> homework5 >> homework6 >> homework7 ;

            // Checks correct input
            cout << "File numerical" << endl ;
            cout << YELLOW << group << " " << surename << " " << name << " " << sex << " " << Q1 << " " << Q2 << " " << Q3 << " " << Q4 << " " << Q5 << " " << Q6 << " " << Q7 << " " << mark << " " << grade << " " << predicted << " " << test1 << " " << test2 << " " << test3 << " " << test4 << " " << test5 << " " << homework1 << " " << homework2 << " " << homework3 << " " << homework4 << " " << homework5 << " " << homework6 << " " << homework7 << " " << RESET << endl ;
    
           
        // Data analysis assign sex to number 1 or 0
        ...
        if ( test1 == 0 )
        {
            ...
            ClassMarkMIN = std::min({test2,test3,test4,test5});
        }
        if ( test2 == 0 )
        {
            ClassMarkMAX = std::max({test1,test3,test4,test5});
            ClassMarkMIN = std::min({test1,test3,test4,test5});
        }
        if ( test3 == 0 )
        {
            ClassMarkMAX = std::max({test1,test2,test4,test5});
            ClassMarkMIN = std::min({test1,test2,test4,test5});
        }
        if ( test4 == 0 )
        {
            ClassMarkMAX = std::max({test1,test2,test3,test5});
            ClassMarkMIN = std::min({test1,test2,test3,test5});
        }
        if ( test5 == 0 )
        {
        ...
        // cout << YELLOW << group << "Class test dispersion (dis): " << ClassMarkDIS << RESET << endl ;
            ...
            //-----------------
            // Topics to revise based on exam results for each topic in the exam
            //-----------------

            std::string ToRevise;
            int CountTopics;
            CountTopics = 0; // Counts the number of topics to be revised
...
//            cout << name <<" should revise " << ToRevise << " length" << ToRevise.length()<< endl ;
            ...
            // Homework checker
            // quality:
            HomeworkAve = ( homework1 + homework2 + homework3 + homework4 + homework5 + homework6 + homework7 )/7;

            // Missing homework if 1 is missing already counts as incomplete
            if ( homework1 == 0 ){HomeworkLack = 1;}
            if ( homework2 == 0 ){HomeworkLack = 1;}
            if ( homework3 == 0 ){HomeworkLack = 1;}
            if ( homework4 == 0 ){HomeworkLack = 1;}
            if ( homework5 == 0 ){HomeworkLack = 1;}
            if ( homework6 == 0 ){HomeworkLack = 1;}
            if ( homework7 == 0 ){HomeworkLack = 1;}
...
//cout << Cheshe[sus] << " consistently maintained " << Excellence[choicesb2] << " standard throughout the year.  " << Cheshe[sus]  << " is a highly capable pupil who grasps new physical concepts with ease.  " << Cheshe[sus]  << " can select and apply equations " << AsRequired[choicesa5] << ".  " ;
            ...
            {
                if ( ClassMarkDIS <= 25 ) //Surprisingly <-> However
                {
                cout << "However, " << hisher[sus] << " class average was consistently significatively below " << hisher[sus] << " grade.  To ensure " << hisher[sus] << " GCSE performance " << heshe[sus] << " must raise the standard of  " << hisher[sus] << " daily work.  " ;
                }

                if ( ClassMarkDIS > 25 ) //Surprisingly <-> However
                {
                cout << "However, " << hisher[sus] << " class standard was irregular and significatively below " << hisher[sus] << " grade.  To ensure " << hisher[sus] << " GCSE performance " << heshe[sus] << " must raise the standard of  " << hisher[sus] << " daily work.  " ;
                }
...
// //             //Class assesment result were irregular and lower throughout the year. 
            ...
            if ( ClassMarkAve >= 90 )
            {
            cout << Cheshe[sus] << " has maintained an extraordinarily high level of excellence throughout the year.  Despite this, " << hisher[sus] << " Summer examination results did not reflect " << hisher[sus] << " hard work and dedication.  ";
          ...
          // Any of this conditions or one or other or etc activates room for improvement
            ...
            if ( 90 > ClassMarkAve && ClassMarkAve >= 80 )
            {
            cout << Cheshe[sus] << " has maintained a consistent very high standard throughout the year, unfortunately this was not fully reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            ...
            if ( 80 > ClassMarkAve && ClassMarkAve >= 70 && ClassMarkDIS <= 25)
            {
            cout << Cheshe[sus] << " has responded positively to the demands of the subject, consistently maintaining a good standard throughout the year.  " ;
            }

            if ( 80 > ClassMarkAve && ClassMarkAve >= 70 && ClassMarkDIS > 25)
            {
            cout << Cheshe[sus] << " has responded positively to the demands of the subject, maintaining an irregular but generally good standard throughout the year.  " ;
            }
            // Idem dispersion or not
            ...
            {
            cout << Cheshe[sus] << " has maintained a consistent and pleasing standard throughout the year.  " << Chisher[sus]  << " steady progression has been reflected in " << hisher[sus] << " Summer assessment.  " ;
            }

            if ( 70 > ClassMarkAve && ClassMarkAve >= 60 && ClassMarkDIS > 25 )
            {
            cout << Cheshe[sus] << " has continued to perform at an irregular but rising standard throughout the year.  " << Chisher[sus] << " progression has been reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            // Idem dispersion or not
            ...
            {
            cout << "Although, " << hisher[sus] << " class marks were consistently low throughout the year, " << heshe[sus]  << " improved " << hisher[sus] << " standard for the Summer assessment.  " ;
            }

            if ( 60 > ClassMarkAve && ClassMarkDIS > 25 )
            {
            cout << "Although, " << hisher[sus] << " class marks were irregular and low throughout the year, " << heshe[sus]  << " improved " << hisher[sus] << " standard for the Summer examination.  " ;
            }
            // Idem dispersion or not
            ...
            if ( ClassMarkAve >= 90 )
            {
            cout << Cheshe[sus] << " has maintained an exceptionally high standard throughout the year, surprisingly this was not reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            
            if ( 90 > ClassMarkAve && ClassMarkAve >= 80 )
            {
            cout << Cheshe[sus] << " has maintained a consistent very high standard throughout the year, surprisingly this was not fully reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            ...
            if ( 80 > ClassMarkAve && ClassMarkAve >= 70 && ClassMarkDIS <= 25)
            {
            cout << Cheshe[sus] << " has maintained a consistent high standard throughout the year, unfortunately this was not fully reflected in " << hisher[sus] << " Summer assessment.  " ;
            }

            if ( 80 > ClassMarkAve && ClassMarkAve >= 70 && ClassMarkDIS > 25)
            {
            cout << Cheshe[sus] << " has maintained a high but slightly irregular standard throughout the year, unfortunately this was not fully reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            // Idem dispersion or not
            ...
            if ( 70 > ClassMarkAve && ClassMarkAve >= 60 && ClassMarkDIS <= 25 )
            {
            cout << Cheshe[sus] << " has maintained a good and consistent standard throughout the year, which has been reflected in " << hisher[sus] << " Summer assessment.  " ;
            }

            if ( 70 > ClassMarkAve && ClassMarkAve >= 60 && ClassMarkDIS > 25 )
            {
            cout << Cheshe[sus] << " has maintained a good but slightly irregular standard throughout the year, which has been reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            // Idem dispersion or not
            ...
            if ( 60 > ClassMarkAve && ClassMarkDIS <= 25 )
            {
            cout << "However, " << hisher[sus] << " performance in class was consistently poor.  " ;
            }
            ...
            if ( ClassMarkAve >= 90 )
            {
            cout << Cheshe[sus] << " has maintained an exceptionally high standard throughout the year, incomprehensibly this was not reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            
            if ( 90 > ClassMarkAve && ClassMarkAve >= 80 )
            {
            cout << "Despite consistently maintaining a very high standard throughout the year, perplexingly, this was not reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            ...
            if ( 80 > ClassMarkAve && ClassMarkAve >= 70 && ClassMarkDIS <= 25)
            {
            cout << Cheshe[sus] << " consistently maintained a high standard throughout the year, unfortunately this was not fully reflected in " << hisher[sus] << " Summer assessment.  " ;
            }

            if ( 80 > ClassMarkAve && ClassMarkAve >= 70 && ClassMarkDIS > 25)
            {
            cout << "Throughout the year, " << heshe[sus] << " maintained a high albeit slightly irregular standard; regrettably, this was not fully reflected in her Summer assessment.  " ;
            } //Throughout the year, her high but somewhat irregular standard was not fully reflected in her Summer assessment.
            // Idem dispersion or not
            ...
            if ( 70 > ClassMarkAve && ClassMarkAve >= 60 && ClassMarkDIS <= 25 )
            {
            cout << Cheshe[sus] << " has maintained a good but slightly irregular standard throughout the year, unfortunately this was not fully reflected in " << hisher[sus] << " Summer assessment.  " ;
            }

            if ( 70 > ClassMarkAve && ClassMarkAve >= 60 && ClassMarkDIS > 25 )
            {
            cout << Cheshe[sus] << " has maintained a good and consistent standard throughout the year, unfortunately this was not fully reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            // Idem dispersion or not
            ...
            if ( 60 > ClassMarkAve && ClassMarkDIS <= 25 )
            {
            cout << Chisher[sus] << " classroom performance remained consistently poor throughout the year.  " ;
            }

            if ( 60 > ClassMarkAve && ClassMarkDIS > 25 )
            {
            cout << "However, " << hisher[sus] << " class performance was consistently poor and irregular throughout the year.  " ;
            }
            // Idem dispersion or not
            ...
            if ( 90 > ClassMarkAve && ClassMarkAve >= 80 )
            {
            cout << Cheshe[sus] << " has maintained a consistent very high standard throughout the year, incomprehensibly this was not reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            ...
            if ( 80 > ClassMarkAve && ClassMarkAve >= 70 && ClassMarkDIS <= 25)
            {
            cout << Cheshe[sus] << " has maintained a consistent high standard throughout the year, surprisingly this was not fully reflected in " << hisher[sus] << " Summer assessment.  " ;
            }

            if ( 80 > ClassMarkAve && ClassMarkAve >= 70 && ClassMarkDIS > 25)
            {
            cout << Cheshe[sus] << " has maintained a high but slightly irregular standard throughout the year, surprisingly this was not fully reflected in " << hisher[sus] << " Summer assessment.  " ;
            }
            // Idem dispersion or not
            ...
            if ( 70 > ClassMarkAve && ClassMarkAve >= 60 && ClassMarkDIS <= 25 )
            {
            cout << Cheshe[sus] << " has maintained a fair and consistent standard throughout the year,  unfortunately " << heshe[sus] << " underperformed in " << hisher[sus] << " Summer assessment.  " ;
            }
